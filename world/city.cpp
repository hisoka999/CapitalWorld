...
City::City(std::string name, utils::Vector2& position) : position(position),name(name),numberOfCitizen(0)
{
...
bool City::isBorderingStreet(std::shared_ptr<Building>& building)
{
    ...
    for(auto& street : streets)
    {
        if(street->get2DPosition().intersectsNoLine(posNorth)
            ||street->get2DPosition().intersectsNoLine(posEast)
            ||street->get2DPosition().intersectsNoLine(posWest)
            ||street->get2DPosition().intersectsNoLine(posSouth))
        {
            isBordering = true;
            break;
        }
